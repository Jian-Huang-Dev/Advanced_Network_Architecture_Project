...
class Discovery (EventMixin):
  ...
  _link_timeout = 2         # How long until we consider a link dead
  ...
  def _expire_links (self):
    ...
    if expired:
      for link in expired:
        ...
        # get link source and destination
        x = int(link._str_src())
        y = int(link._str_dst())
        
        # if link is down, update the weight matrix to break this link
        weights_topo[x][y] = float('+inf')
        weights_topo[y][x] = float('+inf')
  ...
  def _handle_openflow_PacketIn (self, event):
    ...
    if link not in self.adjacency:
      ...
      # once link detected, update weight matrix to revert to original link weights
